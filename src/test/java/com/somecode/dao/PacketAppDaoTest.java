...
public class PacketAppDaoTest extends AbstractDbunitTransactionalJUnit4SpringContextTests {
    /**
     * The default test text prefixes
     */
    ...
    /**
     * The names of the xls files containing the test data
     */
    ...
    private static final String TEST_ADD_COMPTS_BEFORE_FILENAME = "/com/somecode/dao/testAddCompts_before.xls";
    ...
    /**
     * The DAO class constants and variables needed for the testing
     */
    ...
    private static final String PACKET_UPDATE_STATE_UPDATE_NULL_NEW_STATE
            = "PACKET_UPDATE_STATE_UPDATE_NULL_NEW_STATE";
    ...
    /**
     * The Log Appender used for tests of the logging mechanisms
     */
    private TestLogAppender testLogAppender;

    /**
     * The log appender initialization before each test.
     */
    @Before
    ...
    public void testLoadCompts_onePacket_positive() throws Exception {
        ...
        final LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
    ...
    public void testLoadComptsSupplInfo_negative() throws Exception {
        ...
        final int testLogSize = testLogAppender.getLog().size();
        final LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
        assertEquals(Utils.getMessage(testLoadDataForAllSupplInfo, new Object[]{result}), loggingEvent.getMessage());
        assertEquals(Level.INFO, loggingEvent.getLevel());
    }
    ...
    public void testLoadPackets_onePacket_negative() throws Exception {
        ...
        assertEquals(expectedResultSize, result.size());
        int testLogSize = testLogAppender.getLog().size();
        final LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
        assertEquals(Utils.getMessage(testPacketNotFound, new Object[]{packetId}), loggingEvent.getMessage());
    }
    ...
    public void testLoadPackets_allPackets_negative() throws Exception {
        ...
        assertEquals(expectedResultLength, result.size());
        int testLogSize = testLogAppender.getLog().size();
        LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
        assertEquals(Utils.getMessage(testNoPacketsFound, null), loggingEvent.getMessage());
    ...
    public void testUpdateCompts_positive() throws Exception {
        ...
        final List<ComptParams> paramsListForUpdate
                = generateComptParamsList(OperationType.UPDATE, checkedValsForUpdate, idsForUpdate, numOfComptsToUpdate,
                null);
        ...
        {
            assertEquals(Utils.getMessage(testComptUpdateDataComptUpdate,
                    new Object[]{comptIds[i], dataComptIds[i]}),
                    testLogAppender.getLog()
                            .get(testLogSize - 2 - (arrayLength - i - 1))
                            .getMessage());
            assertEquals(Level.INFO, testLogAppender.getLog().get(testLogSize - 2 - (arrayLength - i - 1))
                    .getLevel());
        });
        Long packetId = 1L;
    ...
    public void testAddPackets_positive() throws Exception {
        ...
        final List<ComptParams> comptParamsList = generateComptParamsList(OperationType.ADD, checkedValsForAdding, null,
                numOfComptsToAddInsideAddedPacket,
                labelsForAdding);
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NONE, OperationType.ADD, expectedNewOrUpdatedPacketIdBeforeBeingPersisted,
                packetStateId, comptLabelPrefix, comboDataLabelPrefix, numOfComptsToAddInsideAddedPacket,
                numOfStates, numOfComboDatas, expectedFirstNewComptId,
                expectedPersistedNewOrUpdatedPacketId, secondPacketId);
    }
    ...
    public void testAddPackets_negative_comptsWithEqualLabels() throws Exception {
        ...
        final int numOfStates = 3;
        final int numOfComboDatas = 3;
        final String[][] checkedValsForAdding = generateMultipleLabelsLists(comboDataLabelPrefix, numOfStates,
                numOfComptsToAddInsideAddedPacket);
        final String[] labelsForAdding
        ...
        final List<ComptParams> comptParamsList = generateComptParamsList(OperationType.ADD, checkedValsForAdding, null,
                numOfComptsToAddInsideAddedPacket,
                labelsForAdding);
        final List<PacketParams> packetParamsList = generatePacketParamsList(unusedNewPacketId, packetStateId,
                comptParamsList);
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NONE, OperationType.ADD, expectedNewOrUpdatedPacketIdBeforeBeingPersisted,
                packetStateId, comptLabelPrefix, comboDataLabelPrefix, numOfComptsToAddInsideAddedPacket,
                numOfStates, numOfComboDatas, expectedFirstNewComptId, unusedNewPacketId,
                secondPacketId);
    }
    ...
    public void testAddPacketsSetPacketState_positive_NullStateId() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NULL_NEW_STATE_ID, OperationType.ADD, null, newStateId, comptLabelPrefix,
                comboDataLabelPrefix, numOfCompts, numOfStates, numOfComboDatas, comptId,
    ...
    public void testAddPacketsSetPacketState_positive_nonExistingStateId() throws Exception {
        ...
        final long comptId = 0L;
        final List<ComptParams> comptParamsList = Collections.emptyList();
        final List<PacketParams> packetParamsList = generatePacketParamsList(expectedNewPacketIdBeforeBeingPersisted,
                newStateId, comptParamsList);

        packetAppDao.addOrUpdatePackets(packetParamsList, OperationType.ADD);
        em.flush();

        testAddOrUpdatePackets(PacketDaoErrorType.NOT_EXISTING_STATE_ID, OperationType.ADD,
                expectedNewPacketIdBeforeBeingPersisted, newStateId, comptLabelPrefix,
                comboDataLabelPrefix, numOfCompts, numOfStates, numOfComboDatas, comptId,
                expectedPersistedNewPacketId, secondPacketId);
    }
    ...
    public void testDeletePackets_positive() throws Exception {
        ...
        LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
        assertEquals(Utils.getMessage(testPacketsDeleteSuccessReport, new Object[]{idsToDelete}),
                loggingEvent.getMessage());
        assertEquals(Level.INFO, loggingEvent.getLevel());
    }
    ...
    public void testDeletePackets_negative() throws Exception {
        ...
        assertEquals(expectedLogSize, testLogSize);
        LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - 1);
        assertEquals(Utils.getMessage(testPacketsDeleteNonExistingIds, new Object[]{idsToDelete}),
                loggingEvent.getMessage());
        assertEquals(Level.ERROR, loggingEvent.getLevel());
    }
    ...
    public void testUpdatePacketState_positive() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NONE, OperationType.UPDATE,
                expectedNewOrUpdatedPacketIdBeforeBeingPersisted, newStateId, comptLabelPrefix,
                comboDataLabelPrefix, numOfComptsToAddInsideAddedPacket, numOfStates, numOfComboDatas,
                null, expectedPersistedUpdatedPacketId, secondPacketId);
    }
    ...
    public void testUpdatePacketState_negative_nonExistingPacketId() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NOT_EXISTING_THE_ONLY_PACKET_ID, OperationType.UPDATE, packetId,
                newStateId, comptLabelPrefix, comboDataLabelPrefix, numOfCompts, numOfStates,
                numOfComboDatas, comptId, packetId, secondPacketId);
    }
    ...
    public void testUpdatePacketState_negative_notDifferentNewStateId() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NOT_DIFFERENT_NEW_STATE_ID, OperationType.UPDATE, packetId, newStateId,
                comptLabelPrefix, comboDataLabelPrefix, numOfCompts, numOfStates, numOfComboDatas,
    ...
    public void testUpdatePacketState_negative_nonExistingStateId() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NOT_EXISTING_STATE_ID, OperationType.UPDATE, packetId, newStateId,
                comptLabelPrefix, comboDataLabelPrefix, numOfCompts, numOfStates, numOfComboDatas,
                comptId, packetId, secondPacketId);
    ...
    public void testUpdatePacketState_negative_NullStateId() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NULL_NEW_STATE_ID, OperationType.UPDATE, packetId, newStateId,
                comptLabelPrefix, comboDataLabelPrefix, numOfCompts, numOfStates, numOfComboDatas,
                comptId, packetId, secondPacketId);
    }
    ...
    public void testUpdatePacketAddCompts_positive() throws Exception {
        ...
        final List<ComptParams> paramsListForAdding = generateComptParamsList(OperationType.ADD, checkedValsForAdding,
                null, numOfComptsToAdd, labelsForAdding);
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NONE, OperationType.UPDATE, packetId, stateId, comptLabelPrefix,
                comboDataLabelPrefix, numOfComptsToAdd, numOfStates, numOfComboDatas, firstNewComptId,
    ...
    public void testUpdatePacketAddCompts_negative_nonExistentPacketId() throws Exception {
        ...
        final int numOfStates = 3;
        final String[][] checkedValsForAdding
                = generateMultipleLabelsLists(comboDataLabelPrefix, numOfStates, numOfComptsToAddInsideUnproperlyUpdatedPacket);
        final String[] labelsForAdding
        ...
        IntStream.range(0, numOfComptsToAddInsideUnproperlyUpdatedPacket).boxed().forEach(i ->
                paramsListForAdding.add(new ComptParams().setVals(Arrays.asList(checkedValsForAdding[i]))
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NOT_EXISTING_ONE_OF_PACKET_IDS, OperationType.UPDATE, updatedPacketId,
                updatedPacketNewStateId, comptLabelPrefix, comboDataLabelPrefix,
                numOfComptsToAddInsideProperlyUpdatedPacket, numOfStates, numOfComboDatas,
                expectedFirstNewComptId, updatedPacketId, newPacketId);
    }
    ...
    public void testUpdatePacketAddCompts_negative_comptsWithEqualLabels() throws Exception {
        ...
        testAddOrUpdatePackets(PacketDaoErrorType.NONE, OperationType.ADD, packetId, stateId, comptLabelPrefix,
                comboDataLabelPrefix, numOfComptsToAdd, numOfStates, numOfComboDatas,
                expectedFirstNewComptId, unusedNewPacketId, secondPacketId);
    }
    ...
    public void testDeleteCompts_positive() throws Exception {
        ...
        testDeleteCompts(PacketDaoErrorType.NONE, comptIdsToDelete, fakeIdsSet);
    }
    ...
    public void testDeleteCompts_negative_noneOfTheExistingComptIds() throws Exception {
        ...
        testDeleteCompts(PacketDaoErrorType.NONE_OF_THE_COMPT_IDS_EXIST, comptIdsToDelete, fakeIdsSet);
    }
    ...
    public void testDeleteCompts_negative_someComptIdDoNotExist() throws Exception {
        ...
        testDeleteCompts(PacketDaoErrorType.SOME_OF_THE_COMPT_IDS_EXIST, comptIdsToDelete, fakeIdsSet);
    }
    ...
    public void testGetUserRole_positive() throws Exception {
        ...
        testGetUserRole(PacketDaoErrorType.NONE, username);
    }
    ...
    public void testGetUserRole_negative_invalidUsername() throws Exception {
        ...
        testGetUserRole(PacketDaoErrorType.AUTHENTICATION_ERROR, username);
    }
    ...
    private void loadAndCheckComboData(final Integer expectedResultLength, final String expectedComboDataLabelPrefix)
        ...
        final LoggingEvent comboDataLoadingEvent = testLogAppender.getLog().get(testLogSize - 2);
    ...
    private List<String> generateDiverseLabelsList(String labelPrefix, int numOfLabels) {
        ...
        IntStream.rangeClosed(1, numOfLabels).boxed().forEach(i -> labels.add(labelPrefix + i.toString()));
        return labels;
    }

    private List<String> generateEqualLabelsList(String labelPrefix, int numOfLabels) {
        final List<String> labels = new ArrayList<>();
        IntStream.rangeClosed(1, numOfLabels).forEach(i -> labels.add(labelPrefix + "1"));
        return labels;
    }
    ...
    private List<String> generateIteratedLabelsList(final String labelPrefix,
        ...
        IntStream.rangeClosed(1, numOfIterations).boxed().forEach(i -> labels.addAll(elementaryComboDataList));
    ...
    private long[] generateIds(final int startingId, final int numOfIds) {
        ...
        IntStream.range(startingId, startingId + numOfIds).boxed().forEach(i -> ids[i - startingId] = (long) i);
    ...
    private DatabaseException testEmptyTableCase(DatabaseException exc) {
        ...
        if (exc.getCause().getClass().equals(EmptyComboDataTableException.class)) {
            testEmptyTableExceptionStackTrace
                    = (String) ReflectionTestUtils.getField(packetAppDao, EMPTY_COMBODATA_TABLE_EXCEPTION_STACKTRACE);
            testEmptyTableExceptionMessage
                    = (String) ReflectionTestUtils.getField(packetAppDao, EMPTY_COMBODATA_TABLE_EXCEPTION_MESSAGE);
            ...
            testEmptyTableExceptionStackTrace
                    = (String) ReflectionTestUtils.getField(packetAppDao, EMPTY_STATE_TABLE_EXCEPTION_STACKTRACE);
            testEmptyTableExceptionMessage
                    = (String) ReflectionTestUtils.getField(packetAppDao, EMPTY_STATE_TABLE_EXCEPTION_MESSAGE);
        ...
        assertEquals(Utils.getMessage(testComboDataTableIsEmpty,
                new Object[]{testEmptyTableExceptionMessage,
                        Optional.ofNullable(testEmptyTableExceptionStackTrace).isPresent()
    ...
    private void testDeleteCompts(PacketDaoErrorType error, List<Long> idsToDelete, Set<Long> fakeIds) {
        final int testLogSize = testLogAppender.getLog().size();
        int expectedNumOfLogEventsLeft = 1;

        if (error == PacketDaoErrorType.SOME_OF_THE_COMPT_IDS_EXIST) {
            expectedNumOfLogEventsLeft = 2;
        }

        if (error == PacketDaoErrorType.NONE_OF_THE_COMPT_IDS_EXIST) {
        ...
        assertEquals(Utils.getMessage(testComptsDeleteSuccessReport,
        ...
        if (error == PacketDaoErrorType.SOME_OF_THE_COMPT_IDS_EXIST) {
            ...
            final LoggingEvent someOfIdsExistLoggingEvent
                    = testLogAppender.getLog().get(testLogSize - (expectedNumOfLogEventsLeft--));
            assertEquals(Utils.getMessage(testSomeOfComptIdsExist,
                    new Object[]{fakeIds}),
    ...
    private void testGetUserRole(PacketDaoErrorType error, String username) {
        final int testLogSize = testLogAppender.getLog().size();
        int expectedNumOfLogEventsLeft = 1;

        if (error == PacketDaoErrorType.AUTHENTICATION_ERROR) {
    ...
    private void testLoggingMessage(String errorMessage, int logSize, int expectedNumOfLogEventsLeft, Object errorParam,
                                    Level logLevel) {
        final String testError = (String) ReflectionTestUtils.getField(packetAppDao, errorMessage);
        final LoggingEvent testErrorLogEvent = testLogAppender.getLog().get(logSize - (expectedNumOfLogEventsLeft--));
        assertEquals(Utils.getMessage(testError, new Object[]{errorParam}), testErrorLogEvent.getMessage());
        assertEquals(logLevel, testErrorLogEvent.getLevel());
    }

    private void testAddOrUpdatePackets(PacketDaoErrorType error, OperationType operationType,
        ...
        boolean packetStateIdIsNull = !Optional.ofNullable(packetStateId).isPresent();
        EnumSet<PacketDaoErrorType> fiveElsLogErrorsSet = buildFiveLogErrorsSet();
        EnumSet<PacketDaoErrorType> sixElsLogErrorsSet = buildSixLogErrorsSet();
        if (fiveElsLogErrorsSet.contains(error) && operationType == OperationType.ADD) {
            expectedNumOfLogEventsLeft = 5;
        } else if (operationType == OperationType.UPDATE
                && (error == PacketDaoErrorType.NULL_NEW_STATE_ID || packetStateIdIsNull)
            ...
            expectedNumOfLogEventsLeft = 4;
                ...
                && numOfComptsToAddInsideAddedPacket == 0
                && error != PacketDaoErrorType.NOT_EXISTING_ONE_OF_PACKET_IDS) {
            expectedNumOfLogEventsLeft = 5;
        }
        ...
        if (operationType == OperationType.UPDATE && error == PacketDaoErrorType.NONE && packetStateIdIsNull) {
            LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - (expectedNumOfLogEventsLeft--));
            String testNullNewState
                    = (String) ReflectionTestUtils.getField(packetAppDao,
                    PACKET_UPDATE_STATE_UPDATE_NULL_NEW_STATE);
            assertEquals(Utils.getMessage(testNullNewState,
                    new Object[]{expectedNewOrUpdatedPacketIdBeforeBeingPersisted}),
                    loggingEvent.getMessage());
            assertEquals(Level.ERROR, loggingEvent.getLevel());
        } else if (error == PacketDaoErrorType.NOT_EXISTING_THE_ONLY_PACKET_ID
                || error == PacketDaoErrorType.NOT_EXISTING_ONE_OF_PACKET_IDS) {
            ...
            assertEquals(
                    Utils.getMessage(testPacketUpdateNotExistingPacket,
                            new Object[]{Optional.ofNullable(secondPacketIdIfExists).isPresent()
                                    ? secondPacketIdIfExists
        ...
        } else if (error == PacketDaoErrorType.NOT_DIFFERENT_NEW_STATE_ID) {
            ...
            LoggingEvent loggingEvent = testLogAppender.getLog().get(testLogSize - (expectedNumOfLogEventsLeft--));
            assertEquals(Utils.getMessage(testNotDifferentNewState, new Object[]{packetStateId}),
                    loggingEvent.getMessage());
            assertEquals(Level.ERROR, loggingEvent.getLevel());
            return;
            ...
            if (operationType == OperationType.UPDATE) {
                testNotExistingNewState
                        = (String) ReflectionTestUtils.getField(packetAppDao,
                        PACKET_UPDATE_STATE_UPDATE_NOT_EXISTING_STATE);
                assertEquals(Utils.getMessage(testNotExistingNewState,
                        new Object[]{expectedNewOrUpdatedPacketIdBeforeBeingPersisted,
                                packetStateId}),
                        loggingEvent.getMessage());
                assertEquals(Level.ERROR, loggingEvent.getLevel());
                ...
                testNotExistingNewState
                        ...
                        PACKET_ADDING_STATE_SET_NOT_EXISTING_STATE);
                assertEquals(Utils.getMessage(testNotExistingNewState,
                        new Object[]{packetStateId, defaultPacketState.toString()}),
                        loggingEvent.getMessage());

                assertEquals(Level.INFO, loggingEvent.getLevel());

                testPersistPacketLoggingEvent(testLogSize, expectedNumOfLogEventsLeft,
            ...
            if (operationType == OperationType.ADD) {
                ...
                assertEquals(Utils.getMessage(testNullNewState, new Object[]{defaultPacketState}),
        ...
        if (operationType != OperationType.UPDATE || Optional.ofNullable(packetStateId).isPresent()) {
        ...
        if (numOfComptsToAddInsideAddedPacket > 0) {
            ...
            if (operationType == OperationType.ADD) {
                testPacketAddingOrUpdateAddCompts = (String) ReflectionTestUtils.getField(packetAppDao,
                        PACKET_ADDING_ADD_COMPTS);
            } else if (operationType == OperationType.UPDATE) {
                testPacketAddingOrUpdateAddCompts = (String) ReflectionTestUtils.getField(packetAppDao,
                        PACKET_UPDATE_ADD_COMPTS);
            }
            ...
            for (long j = 1; j < (numOfComboDatas + 1); j++) {
            ...
            assertEquals(Utils.getMessage(testPacketAddingOrUpdateAddCompts, messageArgs),
        ...
        final LoggingEvent persistPacketLoggingEvent
                = testLogAppender.getLog().get(testLogSize - (expectedNumOfLogEventsLeft--));
        List<Compt> packetComptsList = new ArrayList<>();

        if (Optional.ofNullable(expectedFirstNewComptId).isPresent()) {
        ...
        assertEquals(
                Utils.getMessage(testPacketAddingOrUpdateSuccessReport, new Object[]{packet}),
    ...
    private EnumSet<PacketDaoErrorType> buildFiveLogErrorsSet() {
        EnumSet<PacketDaoErrorType> fiveElsLogErrorsSet = EnumSet.noneOf(PacketDaoErrorType.class);
        fiveElsLogErrorsSet.add(PacketDaoErrorType.NULL_NEW_STATE_ID);
        fiveElsLogErrorsSet.add(PacketDaoErrorType.NOT_EXISTING_STATE_ID);
        return fiveElsLogErrorsSet;
    }

    private EnumSet<PacketDaoErrorType> buildSixLogErrorsSet() {
        EnumSet<PacketDaoErrorType> fiveElsLogErrorsSet = EnumSet.noneOf(PacketDaoErrorType.class);
        fiveElsLogErrorsSet.add(PacketDaoErrorType.NONE);
        fiveElsLogErrorsSet.add(PacketDaoErrorType.NOT_EXISTING_ONE_OF_PACKET_IDS);
        return fiveElsLogErrorsSet;
    }
}
